...
### 2026-04-26 – README.md + info.md rewrite for v4.0.0-alpha.1
- **README rewritten end-to-end** to match the entities that actually exist on disk and have been verified on the YAS-209: 1 media_player, 8 switches, 2 selects, 2 numbers, 8 sensors. Section structure exactly per the prompt's 13-item list (Title + badge → Compatibility → Installation → Configuration (UI) → Entities → Upgrading → Known limitations → Network → Security note → Troubleshooting → Contributing → License). Markdown tables used for switches and sensors (enumerable); bullet lists for selects and numbers. Compatibility expressed as a table making the "verified vs. expected" distinction explicit per model.
- **Stale references removed:** the old "Configuration (YAML, legacy)" section (which presented YAML setup as a current option) is gone; the YAML block now appears only inside the v3 → v4 upgrading instructions as a "delete this from configuration.yaml" example. The four-step Installation list that ended with "Add the configuration to your configuration.yaml" was rewritten to UI-first (HACS path then manual). The `sound_settings` service block was dropped — it still exists in `services.py` but it's a v3 holdover that the per-feature switches/numbers cover, and the prompt's explicit structure list didn't include a Services section.
- **Stale references in `info.md`** (manual install path typo `yamha_soundbar`, no UI setup mention, sound_settings service section) replaced with a five-paragraph HACS landing page: description, compatibility, entity counts, install/setup steps, link to README.
- **Hardware findings written exactly per the session log:** dimmer is read-only on YAS-209 (`accepts and returns OK but doesn't actually change`), `sport` (singular) silently fails, Audio Stream typically `OFF` on TVs without passthrough, input mode codes are YAS-209-specific. Net Standby's "disabled by default — toggling off can soft-brick" warning kept inline in the switches table. Firmware sensors marked diagnostic + disabled-by-default.
- **Preserved verbatim:** the HACS badge, the Network section's DHCP-reservation paragraph, the Security note (full Yamaha-cert provenance + osk2 medium link), the License section pointing at LICENSE.
- **Verifications:** `python -c "open('README.md').read()"` and `open('info.md').read()` both ok; grep over the entity-name set found 21 mentions in README confirming every entity from the ground truth is named. Section heading audit confirmed all 13 prompt-required sections present, in order, no extras.
- **Files touched this session:** `README.md` (rewritten), `info.md` (rewritten), `AGENT_HANDOFF.md` (this entry). No `.py`, no `manifest.json`, no `hacs.json`, no `strings.json`, no LICENSE.

### 2026-04-26 – input_mode ENUM sensor (read-only, parallel to input_source select)
- **Built one entity:** `input_mode` sensor in `sensor.py`, `device_class=SensorDeviceClass.ENUM`, `options=["Bluetooth", "TV", "HDMI", "Net"]`. Reads `coordinator.data["player"]["mode"]`, int-coerces, looks up in a literal `mode_map={41:"Bluetooth", 43:"TV", 49:"HDMI", 31:"Net"}`. Returns `None` for missing key, non-numeric value, or unknown integer (no raises).
- **Why duplicate select.py's read side:** the existing `input_source` select in `select.py` is the *control* — it's how a user picks an input. This new sensor is the *trigger* — it's how an automation reacts to an input change. HA automations bind cleanly to sensor state changes (`platform: state`, `entity_id: sensor.foo, to: "HDMI"`); reaching into a select's `current_option` attribute is uglier and more brittle. The select and the sensor read the same underlying field but serve different roles.
- **Architecture additions:** new `YamahaPlayerMappedSensorDescription` dataclass (sibling to the existing three description types), with fields `read_field: str` + `mode_map: dict[int, str]`. New tuple `PLAYER_MAPPED_SENSORS` (one entry today). New entity class `YamahaPlayerMappedSensor` (inherits `_YamahaSensorBase`, overrides `native_value`). `async_setup_entry` extended to register the new tuple alongside the three existing sensor families.
- **Intentional non-abstraction:** the four-mode mapping is duplicated between `select.py`'s `SELECTS[0].mode_map` (which uses `(label, set_verb)` tuples — needed for write) and `sensor.py`'s `PLAYER_MAPPED_SENSORS[0].mode_map` (just labels — read-only). The shapes differ enough that pulling them into a shared constants module would cost more than it saves, and would force every future caller to know which projection it wanted. Module-level comments in `sensor.py` (above the `PLAYER_MAPPED_SENSORS` tuple) and the `YamahaPlayerMappedSensorDescription` docstring both flag that any future mode-int addition must be made in BOTH files.
- **Translations:** added `entity.sensor.input_mode.name = "Input Mode"` to both `strings.json` and `translations/en.json`. The four state values come automatically from `SensorDeviceClass.ENUM` + `options`, so no per-state translation entries needed.
- **Tests:** extended `tests/test_sensor.py` with 12 new tests in a clearly-marked Player-mapped sensor section: registry guard (one entry), `options` and `read_field` pin, ENUM device_class pin, full `mode_map` dict pin (so a future edit fails loudly), parametrized `native_value` over all 4 (mode, label) pairs as both string `"41"` and int `41` forms (8 cases total), unknown-int → None, unparsable string → None, missing-mode-key → None, missing-player → None, None-data → None, unique_id pin, translation_key + has_entity_name pin. Existing 19 sensor tests are unchanged.
- **Verifications:**
  - `python -m compileall -f custom_components/yamaha_soundbar` → all 13 modules clean.
  - JSON parse on `strings.json` and `translations/en.json` → ok.
  - AST parse on `sensor.py` and `tests/test_sensor.py` → ok.
  - `python -m pytest tests/test_sensor.py -v` → **skipped**: env Python 3.9.7, `api.py` uses `dataclass(slots=True)` (3.10+), collection errors at import. Same situation as every prior session; tests run cleanly under CI's 3.12.
- **Not touched (per prompt rules):** `select.py`, `number.py`, `switch.py`, `coordinator.py`, `api.py`, `media_player.py`, `_yamaha_codec.py`, `__init__.py`, `const.py`, `config_flow.py`, `services.py`, `entity.py`, `manifest.json`, `hacs.json`, conftest, all other test files. Only `sensor.py`, `strings.json`, `translations/en.json`, `tests/test_sensor.py`, and `AGENT_HANDOFF.md` modified this session.
